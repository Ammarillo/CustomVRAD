...
void GatherLight (int threadnum, void *pUserData)
{
	...
	while (1)
	{
		...
		patch = &g_Patches[j];
		const int receiverEnv = bTintInbound ? LightEnv_GetPatchEnvId( j ) : LIGHTENV_ID_DEFAULT;
...
void BounceLight (void)
{
	...
	{
		Msg( "Building light_environment_volume patch env cache for inbound bounce tinting...\n" );
		LightEnv_BuildPatchEnvCache();
	}
	...
	if ( VRadGPU_IsActive() )
	{
		...
		if ( !bAnyBump )
		{
			CUtlVector<Vector> reflectivity;
			CUtlVector<VRadGPUTransfer_t> transfers;
			CUtlVector<int> offsets;
			CUtlVector<int> patchEnv;
			reflectivity.SetCount( (int)uiPatchCount );
			patchEnv.SetCount( (int)uiPatchCount );
			offsets.SetCount( (int)uiPatchCount + 1 );
			offsets[0] = 0;
			for ( unsigned int p = 0; p < uiPatchCount; ++p )
			{
				reflectivity[p] = g_Patches[p].reflectivity;
				patchEnv[p] = LightEnv_GetPatchEnvId( (int)p );
				const int n = g_Patches[p].numtransfers;
				for ( int t = 0; t < n; ++t )
				{
			...
			if ( VRadGPU_UploadTransferGraph( reflectivity.Base(), transfers.Base(),
											  offsets.Base(), patchEnv.Base(), (int)uiPatchCount ) )
			{
				gpuAddOut.SetCount( (int)uiPatchCount );
				bGPUGatherReady = true;
			}
		}
	}
	...
	while ( bouncing )
	{
		...
		bool bGPUGather = false;
		if ( bGPUGatherReady &&
			 VRadGPU_GatherBounce( emitlight.Base(), gpuAddOut.Base(), (int)uiPatchCount ) )
		{
			for ( unsigned int p = 0; p < uiPatchCount; ++p )
				VectorCopy( gpuAddOut[p], addlight[p].light[0] );
			bGPUGather = true;
		}
		if ( !bGPUGather )
...
int ParseCommandLine( int argc, char **argv, bool *onlydetail )
{
	*onlydetail = false;
	...
	for( i=1 ; i<argc ; i++ )
	{
		...
		{
			g_bVRadCoarsePatches = true;
			maxchop = 8;
			minchop = 8;
			Msg( "Coarse patches (-coarse): chop=8 (fewer patches, faster VisLeafs/bounce).\n" );
		}
		else if ( !Q_stricmp( argv[i], "-bounce_soft" ) )
		{
			if ( ++i < argc )
			{
				g_flBounceRadialScale = (float)atof( argv[i] );
				if ( g_flBounceRadialScale < 0.5f )
					g_flBounceRadialScale = 0.5f;
				if ( g_flBounceRadialScale > 4.0f )
					g_flBounceRadialScale = 4.0f;
		...
		else if ( !Q_stricmp( argv[i], "-maxtransfer" ) )
		{
			if ( ++i < argc )
			{
				g_flMaxTransferDist = (float)atof( argv[i] );
				Msg( "Max transfer distance (-maxtransfer): %.0f units.\n", g_flMaxTransferDist );
			}
		}
		...
		else if ( !Q_stricmp( argv[i], "-ao_samples" ) )
		{
			if ( ++i < argc )
			{
				g_nAOSamples = atoi( argv[i] );
				if ( g_nAOSamples < 1 )
					g_nAOSamples = 1;
				g_bAO = true;
				Msg( "AO samples (-ao_samples): %d\n", g_nAOSamples );
			}
		}
		else if ( !Q_stricmp( argv[i], "-ao_distance" ) )
		{
			if ( ++i < argc )
			{
				g_flAODistance = (float)atof( argv[i] );
				g_bAO = true;
				Msg( "AO distance (-ao_distance): %.1f\n", g_flAODistance );
			}
		}
		else if ( !Q_stricmp( argv[i], "-ao_strength" ) )
		{
			if ( ++i < argc )
			{
				g_flAOStrength = (float)atof( argv[i] );
				g_bAO = true;
				Msg( "AO strength (-ao_strength): %.2f\n", g_flAOStrength );
			}
		}
		else if ( !Q_stricmp( argv[i], "-ao_bias" ) )
		{
			if ( ++i < argc )
			{
				g_flAOBias = (float)atof( argv[i] );
				g_bAO = true;
				Msg( "AO bias (-ao_bias): %.2f\n", g_flAOBias );
			}
		}
		else if ( !Q_stricmp( argv[i], "-ao_denoise" ) )
		{
			g_bAODenoise = true;
			g_bAO = true;
			Msg( "AO denoise (-ao_denoise): on (radius=%d strength=%.2f)\n",
				 g_nAODenoiseRadius, g_flAODenoiseStrength );
		}
		else if ( !Q_stricmp( argv[i], "-ao_denoise_radius" ) )
		{
			if ( ++i < argc )
			{
				g_nAODenoiseRadius = atoi( argv[i] );
				g_bAODenoise = true;
				g_bAO = true;
				if ( g_nAODenoiseRadius < 1 ) g_nAODenoiseRadius = 1;
				if ( g_nAODenoiseRadius > 4 ) g_nAODenoiseRadius = 4;
				Msg( "AO denoise radius (-ao_denoise_radius): %d\n", g_nAODenoiseRadius );
			}
		}
		else if ( !Q_stricmp( argv[i], "-ao_denoise_strength" ) )
		{
			if ( ++i < argc )
			{
				g_flAODenoiseStrength = (float)atof( argv[i] );
				g_bAODenoise = true;
				g_bAO = true;
				if ( g_flAODenoiseStrength < 0.0f ) g_flAODenoiseStrength = 0.0f;
				if ( g_flAODenoiseStrength > 1.0f ) g_flAODenoiseStrength = 1.0f;
				Msg( "AO denoise strength (-ao_denoise_strength): %.2f\n", g_flAODenoiseStrength );
			}
		}
		...
		else if (!Q_stricmp(argv[i],"-threads"))
		{
			if ( ++i < argc )
			{
				...
				{
					Warning( "Clamping -threads from %d to %d\n", numthreads, MAX_TOOL_THREADS );
					numthreads = MAX_TOOL_THREADS;
				}
			}
			else
...
void PrintUsage( int argc, char **argv )
{
	...
	Warning(	
		...
		"  -ao_bias N      : Surface offset along normal (default 0.25). Implies -ao.\n"
		"  -ao_denoise     : Edge-preserving AO denoise filter. Implies -ao.\n"
		"  -ao_denoise_radius N : Denoise radius 1..4 (default 1). Implies -ao_denoise.\n"
		"  -ao_denoise_strength N : Denoise blend 0..1 (default 1). Implies -ao_denoise.\n"
		"\n"
