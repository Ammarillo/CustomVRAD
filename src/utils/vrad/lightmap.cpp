...
static void ParseSkyAmbientFromEntity( entity_t* e, directlight_t *pSky, directlight_t *pAmbient )
{
	if( g_bHDR && LightForKey( e, "_ambientHDR", pAmbient->light.intensity ) )
	{
		// we have a valid HDR ambient light value
	}
	else if ( !LightForKey( e, "_ambient", pAmbient->light.intensity ) )
	{
		VectorScale( pSky->light.intensity, 0.5, pAmbient->light.intensity );
	}
	if ( g_bHDR )
	{
		VectorScale( pAmbient->light.intensity,
...
static float ParseSunSpreadExtent( entity_t* e )
{
	...
	if ( !angle_str || !angle_str[0] )
		return g_SunAngularExtent;

	float extent = (float)atof( angle_str );
	return (float)sin( ( M_PI / 180.0 ) * extent );
}
...
static void ParseLightEnvironment( entity_t* e, directlight_t* dl )
{
	...
	float sunExtent = ParseSunSpreadExtent( e );
	if ( ValueForKeyWithDefault( e, "SunSpreadAngle" ) )
	{
		g_SunAngularExtent = sunExtent;
		printf("sun extent from map=%f\n", g_SunAngularExtent );
	}
	dl->m_flSunAngularExtent = g_SunAngularExtent;
	...
	if ( !gSkyLight )
	{
		...
		dl->m_nEnvId = LIGHTENV_ID_DEFAULT;

		// Sky ambient light.
		gAmbient = AllocDLight( dl->light.origin, false );
		gAmbient->light.type = emit_skyambient;
		gAmbient->m_nEnvId = LIGHTENV_ID_DEFAULT;
		ParseSkyAmbientFromEntity( e, dl, gAmbient );
		LightEnv_SetDefaultBounceIntensity( dl->light.intensity );
...
static void ParseLightEnvironmentVolume( entity_t* e )
{
	char *pModel = ValueForKey( e, "model" );
	if ( !pModel || pModel[0] != '*' )
	{
		Warning( "WARNING: light_env_vol without brush model ignored\n" );
		return;
	}
	...
	{
		Warning( "WARNING: light_env_vol has invalid model \"%s\"\n", pModel );
		return;
	}

	dmodel_t *pModelData = &dmodels[modelIndex];
	float blendDistance = FloatForKeyWithDefault( e, "BlendDistance", 0.0f );
	int blendMode = IntForKeyWithDefault( e, "BlendMode", LIGHTENV_BLEND_CENTER );
	int priority = IntForKeyWithDefault( e, "priority", 0 );
	// Key names must stay under MAX_KEY (32). Accept legacy long names too.
	bool inboundBounceUsesVolumeColor =
	...
	Vector dest;
	GetVectorForKey( e, "origin", dest );
	if ( dest.LengthSqr() < 1e-4f )
	{
		VectorAdd( pModelData->mins, pModelData->maxs, dest );
		VectorScale( dest, 0.5f, dest );
	}

	directlight_t *pSky = AllocDLight( dest, false );
	ParseLightGeneric( e, pSky );
	...
	if ( blendMode == LIGHTENV_BLEND_INSIDE )
...
void CreateDirectLights (void)
{
	...
	for (i=0 ; i<(unsigned)num_entities ; i++)
	{
		e = &entities[i];
		name = ValueForKey (e, "classname");
		// light_env_vol = short classname; light_environment_volume = legacy
		if (!strcmp(name, "light_env_vol") || !strcmp(name, "light_environment_volume"))
			ParseLightEnvironmentVolume( e );
	}

	// AO: global point entity then volumes (map settings override CLI defaults).
	for (i=0 ; i<(unsigned)num_entities ; i++)
	{
		e = &entities[i];
		name = ValueForKey (e, "classname");
		if (!strcmp(name, "light_ao"))
			AO_ParseGlobalEntity( e );
	}
	for (i=0 ; i<(unsigned)num_entities ; i++)
	{
		e = &entities[i];
		name = ValueForKey (e, "classname");
		if (!strcmp(name, "light_ao_vol"))
			AO_ParseVolumeEntity( e );
	}
	if ( AO_HasVolumes() )
		Msg( "light_ao_vol: %d volume(s) active\n", AO_VolumeCount() );
	...
	LightEnv_CopySkyVisToVolumeLights();

	if ( LightEnv_HasVolumes() )
	{
		Msg( "light_env_vol: %d volume(s) active\n", LightEnv_VolumeCount() );
	}
...
void ExportDirectLightsToWorldLights()
{
	...
	*pNumworldlights = 0;
	...
	{
		// Volume environments are compile-only; do not export to engine worldlights.
	...
	if ( VRadGPU_HasScene() && nsamples >= 8 && !g_bTextureShadows )
	{
		...
		for ( int d = 0; d < nsamples; d++ )
		{
			...
			for ( int lane = 0; lane < 4; lane++ )
			{
				...
				ends.push_back( pos.Vec( lane ) + delta );
			}
		}
		std::vector<float> hitT( starts.size() );
...
// GPU-batched ambient sky (replaces per-ray TestLine_DoesHitSky when BVH is ready).
static bool GatherSampleAmbientSkySSE_GPU( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum,
	FourVectors const& pos, FourVectors *pNormals, int normalCount, int iThread,
	int nLFlags, int static_prop_index_to_ignore, float flEpsilon )
{
	if ( !VRadGPU_HasScene() )
		return false;
	...
	{
		fltx4 dots[NUM_BUMP_VECTS+1];
		int rayIndex[4]; // -1 if lane unused
	};
	std::vector<AmbSample_t> samples;
	samples.reserve( nsky_samples );
	std::vector<Vector> starts, ends;
	starts.reserve( nsky_samples * 4 );
	ends.reserve( nsky_samples * 4 );
	...
	for ( int j = 0; j < nsky_samples; j++ )
	{
		...
		for ( int lane = 0; lane < 4; lane++ )
		{
			if ( SubFloat( validity, lane ) > 0.0f )
			{
				s.rayIndex[lane] = (int)starts.size();
				starts.push_back( surfacePos.Vec( lane ) );
				ends.push_back( delta.Vec( lane ) );
			}
			else
				s.rayIndex[lane] = -1;
		}
	...
	for ( size_t si = 0; si < samples.size(); ++si )
	{
		...
		for ( int lane = 0; lane < 4; lane++ )
		{
			int ri = s.rayIndex[lane];
			if ( ri < 0 )
				continue;
			Vector d = ends[ri] - starts[ri];
			float tmax = d.Length();
		...
		for ( int i = 0; i < normalCount; i++ )
		{
			fltx4 addedAmount = MulSIMD( fractionVisible, s.dots[i] );
			ambient_intensity[i] = AddSIMD( ambient_intensity[i], addedAmount );
		}
	}
					   ...
					   float flEpsilon )
{
	...
	const bool bEnvLight = ( dl->light.type == emit_skylight || dl->light.type == emit_skyambient );
	fltx4 envWeights = Four_Ones;
	if ( bEnvLight && LightEnv_HasVolumes() )
	{
		// Thread-local cache: all env lights at a sample group share the same 4 positions.
		// Rebuild once per unique position set instead of walking volumes per light.
		struct EnvWeightCache_t
		{
			Vector pos[4];
			int nEnvIds; // VolumeCount()+1
			float w[LIGHTENV_MAX_VOLUMES + 1][4];
			bool valid;
		};
		static thread_local EnvWeightCache_t s_cache = {};

		Vector p0 = pos.Vec( 0 );
		Vector p1 = pos.Vec( 1 );
		Vector p2 = pos.Vec( 2 );
		Vector p3 = pos.Vec( 3 );
		const int nEnvIds = LightEnv_VolumeCount() + 1;

		bool bMatch = s_cache.valid && s_cache.nEnvIds == nEnvIds &&
			s_cache.pos[0] == p0 && s_cache.pos[1] == p1 &&
			s_cache.pos[2] == p2 && s_cache.pos[3] == p3;
		...
		if ( !bMatch )
		{
			...
			s_cache.pos[2] = p2;
			s_cache.pos[3] = p3;
			s_cache.nEnvIds = nEnvIds;
			float tmp[LIGHTENV_MAX_VOLUMES + 1];
			LightEnv_ComputeAllWeights( p0, tmp );
			for ( int e = 0; e < nEnvIds; ++e ) s_cache.w[e][0] = tmp[e];
			LightEnv_ComputeAllWeights( p1, tmp );
			for ( int e = 0; e < nEnvIds; ++e ) s_cache.w[e][1] = tmp[e];
			LightEnv_ComputeAllWeights( p2, tmp );
			for ( int e = 0; e < nEnvIds; ++e ) s_cache.w[e][2] = tmp[e];
			LightEnv_ComputeAllWeights( p3, tmp );
			for ( int e = 0; e < nEnvIds; ++e ) s_cache.w[e][3] = tmp[e];
			s_cache.valid = true;
		}

		const int envId = dl->m_nEnvId;
		if ( envId < 0 || envId >= nEnvIds )
			return;

		float w0 = s_cache.w[envId][0];
		float w1 = s_cache.w[envId][1];
		float w2 = s_cache.w[envId][2];
		float w3 = s_cache.w[envId][3];
		if ( w0 <= 0.0f && w1 <= 0.0f && w2 <= 0.0f && w3 <= 0.0f )
			return;
		envWeights = Four_Zeros;
		envWeights = SetComponentSIMD( envWeights, 0, w0 );
		envWeights = SetComponentSIMD( envWeights, 1, w1 );
		envWeights = SetComponentSIMD( envWeights, 2, w2 );
		envWeights = SetComponentSIMD( envWeights, 3, w3 );
	}
	...
	if ( bEnvLight && LightEnv_HasVolumes() )
	{
		out.m_flFalloff = MulSIMD( out.m_flFalloff, envWeights );
