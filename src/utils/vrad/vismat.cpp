...
void TestPatchToFace (unsigned patchnum, int facenum, int head, transfer_t *transfers, CTransferMaker &transferMaker, int iThread )
{
	...
	if ( patch2 && DotProduct(patch->origin, patch2->normal) > patch2->planeDist + PLANE_TEST_EPSILON )
	{
		...
		for( ; patch2; patch2 = pNextPatch )
		{
			...
			// Optional distance cull (-maxtransfer). Cuts transfer count on large maps.
			if ( g_flMaxTransferDist > 0.0f )
			{
				Vector tmpDist;
				VectorSubtract( patch->origin, patch2->origin, tmpDist );
				if ( DotProduct( tmpDist, tmpDist ) > g_flMaxTransferDist * g_flMaxTransferDist )
					continue;
			}
