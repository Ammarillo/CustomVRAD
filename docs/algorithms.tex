...
\title{%
  \vspace{-1.2em}
  {\color{cvBlue}\Huge\bfseries PathRAD}\\[0.35em]
...
\begin{abstract}
\noindent
PathRAD is a 64-bit VRAD fork for Garry's~Mod (Source~SDK~2013).
It still has Valve's patch-radiosity path, and you can swap world-face
(and static-prop) lighting for a DirectX~Raytracing (DXR) path tracer.
This paper writes down how those bakes actually work: what is estimated,
how coordinates are defined, what the flags mean, and how the two pipelines
relate.

Also covered: hero-wavelength spectral transport, contact-hardening soft
shadows, textured emissive and filter materials, lighting volumes, ambient
occlusion, IES spots and projected textures, underwater media
(Beer--Lambert / Kd and homogeneous volume path tracing), volumetric fog
grids packed into the BSP, texture shadows on \vmt{alphatest}/\vmt{translucent}
props, and lightmap post-process (denoise, seam stitch, edge pull).
\end{abstract}
...
PathRAD writes HDR and LDR lightmaps the Source engine can load.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Path tracing} --- D3D12 DXR baker (\flag{-pathtrace} /
        ...
        \flag{-StaticPropLighting}. Detail props use the same GPU baker
        (one sample at the sprite or model centre).
\end{enumerate}

Leaf ambient cubes and engine-exported worldlights stay on
stock paths, with PathRAD volume extras where noted.
Detail props use the GPU path tracer when the world bake succeeded
(same spp/bounces as prop verts). If that baker is unavailable, they
sample nearby world lightmaps and do not add classic direct (which
would double-count the sun).
If DXR setup or the bake fails, the compiler falls back to classic
...
\subsection{\texttt{bake\_volume} local rebake}

If any enabled \ent{bake\_volume} brush is present (and \flag{-nobakevolume}
was not passed), only lighting \emph{samples} whose world position lies inside
the (padded) volume AABB are recomputed. World luxels and static-prop vertices
outside the AABB are copied from the previous compile
(\texttt{mapsrc/<map>.vradlm}). A face or prop that straddles the volume is
updated only on the interior samples.

The ray-tracing scene is unchanged: geometry outside the volume still occludes
and bounces into interior samples; it is simply not itself written. Faces or
vertices with no cache hit are still baked so they cannot go black.
\flag{-nobakevolume} forces a full map bake (and still refreshes the cache).
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Upsampling:} Smits~(1999) combination as in PBRT-v3.
        CPU tables are $32$ samples; the GPU baker uses $16$ knots (even samples
        plus $720\,\mathrm{nm}$) so DXC stays stable. Separate illuminant and
        reflectance bases. A previous GPU Gaussian-lobe upsample mixed green
        into saturated red (lights looked orange); Smits red is near-zero below
        $\sim 580\,\mathrm{nm}$.
  ...
  \item \textbf{Projection:} Wyman analytic CIE~1931 $2^\circ$ CMFs $\to$ XYZ,
        then IEC~61966-2-1 / Rec.~709 matrix to linear sRGB, scaled so
        $\mathrm{SpecIllum}(1,1,1)$ round-trips to neutral RGB.
\end{itemize}
...
at the centre (softness is bake-only). Optional \code{LightTemperature} as in
\cref{sec:cct}.
...
\code{lua\_run} stubs that hook \code{RenderScreenspaceEffects}.
...
Participating-medium bake for BSP water
...
\begin{tabular}{@{}lll@{}}
  ...
  \flag{-underwater} & A & Beer--Lambert segment $T$ $+$ Kd downwelling for sky/sun \\
	\flag{-uw\_volume} & B & Homogeneous free-flight volume PT (implies A media) \\
	\flag{-uw\_off} & --- & Force disable \\
	\flag{-uw\_maxscatter}~$N$ & B cap & Max medium events (default~$8$, max~$64$) \\
  \bottomrule
\end{tabular}
\end{center}
...
(double counting). Local lights use segment $T$ only. Mode~B disables Kd.

\subsection{Mode B (volume path tracing)}
...
\subsection{Color temperature}
\label{sec:cct}

Optional entity keys \code{LightTemperature} and \code{AmbientTemperature}
(Kelvin). A value $\le 0$ leaves authored RGB unchanged. Otherwise the
correlated colour temperature is converted to sRGB with the Tanner Helland
approximation of Mitchell Charity's blackbody locus (clamped to
$1000\,\mathrm{K}$--$40000\,\mathrm{K}$), decoded with the same $\gamma=2.2$
as Hammer \code{\_light} colours, then scaled to preserve Rec.~709 luminance
of the existing intensity so brightness (the fourth \code{\_light} number)
is unchanged. The tinted linear RGB is used for the bake and for exported
worldlights. \code{AmbientTemperature} applies to \code{\_ambient} on
\ent{light\_environment} / \ent{light\_env\_vol}.
...
Intel Open Image Denoise \code{RTLightmap} model on CPU (\texttt{hdr} on).
One thread-local filter per bake worker (pinned to avoid oversubscription).
Charts smaller than $16^{2}$ are edge-clamp padded.
After world faces finish, coplanar connected faces that share a lightmap
frame are packed into one island image and denoised together so OIDN sees
neighbour context across VBSP splits. Displacements and oversized islands
($>2048$ on a side) stay per-face. Sakai remains per-face (needs variance).
...
On by default after \code{FinalLightFace}. Coplanar faces whose edges
geometrically overlap (identical verts, T-junctions, or duplicated verts)
blend near-edge luxels toward the neighbour value at the same world position
so both sides converge to a common average. Per lightstyle and bump layer;
displacements skipped. Disable with \flag{-nostitch}.
...
\begin{tabular}{@{}llp{5.8cm}@{}}
  ...
  \flag{-uw\_volume} & off & Mode~B free-flight volume PT \\
  \flag{-uw\_maxscatter}~$N$ & $8$ & Cap medium scatters (Mode~B) \\
  ...
  \flag{-nobakevolume} & --- & Ignore \texttt{bake\_volume}; full map rebake \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}ll@{}}
  ...
  \code{pt\_spectral.h} & Smits bases, CIE CMFs, RGB$\leftrightarrow\lambda$ (CPU) \\
  \code{pt\_spectral\_hlsl.cpp} & GPU 16-knot Smits + CIE injection \\
  ...
  \code{bake\_volume.*} & Local AABB rebake (per-luxel / per-vert) + cache \\
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Tanner Helland / Mitchell Charity --- blackbody (Planckian) RGB approximation
...
\noindent
This matches the code in the PathRAD tree.
Flag defaults can change; trust \texttt{vrad.exe} help and
\texttt{configs/full.cfg} for what actually ships.
}
