...
\title{%
  ...
  {\Large Algorithm Reference}\\[0.5em]
  {\normalsize\color{cvMuted} Path-traced and radiosity lightmap baking for Source~SDK~2013 / Garry's~Mod}
}
...
\begin{abstract}
\noindent
CustomVRAD is a drop-in 64-bit replacement for Valve's VRAD lightmap compiler.
It extends the classic patch radiosity pipeline with an optional DirectX~Raytracing
(DXR) path tracer, hero-wavelength spectral transport, contact-hardening soft shadows,
textured emissive and filter materials, volume overrides, multi-scale ambient occlusion,
and several lightmap-quality post-processes.
This document describes the implemented algorithms, their mathematical formulation,
parameter semantics, and the relationship between the classic and path-traced bake modes.
\end{abstract}
...
Two mutually exclusive world-face lighting pipelines exist:

\begin{enumerate}[leftmargin=1.6em]
  \item \textbf{Classic radiosity} --- stock VRAD \code{BuildFacelights} plus iterative
        patch gather, optionally accelerated by OpenCL (\flag{-gpu}).
  \item \textbf{Path tracing} --- D3D12 DXR baker activated by \flag{-pathtrace} /
        \flag{-dxr}. On success it replaces world-face direct lighting \emph{and}
        radiosity bounce. Static props can share the same integrator under
        \flag{-StaticPropLighting}.
\end{enumerate}

Leaf ambient cubes, detail props, and engine-exported worldlights remain on stock
paths (with CustomVRAD volume-aware extensions where noted).
Failure of DXR init or bake falls back to classic radiosity for world faces
(and CPU prop gather for props).

\begin{notebox}[Two GPU paths]
  \flag{-pt\_gpu} drives the DXR RayQuery luxel baker.
  \flag{-gpu} is an independent OpenCL path that accelerates classic radiosity
  gather and batched AO occlusion. They are not interchangeable.
\end{notebox}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Vertex lighting} --- virtual per-triangle lightmap with edge
        subdivision \flag{-pt\_prop\_vertgrid}~$G$ (default~$4$):
        \begin{equation}
          \#\text{samples/triangle}
          =
          \frac{(G+1)(G+2)}{2}.
        \end{equation}
        Lattice samples scatter to the three vertices by barycentric weights;
        each vertex normalises by accumulated weight. Unlit verts inherit from
...
\begin{tabular}{@{}llp{5.8cm}@{}}
  ...
  \flag{-pt\_prop\_vertgrid} & $4$ & Prop vertex lattice \\
...
\begin{tabular}{@{}ll@{}}
  ...
  \code{vradstaticprops.cpp} & Prop SSE + pathtrace vertgrid \\
  \code{lightmap.cpp} & Edge pull, final face lighting \\
  \bottomrule
\end{tabular}
\end{center}
...
\noindent
This document describes algorithms as implemented in the CustomVRAD tree.
Defaults and clamps may change; the CLI help from \texttt{vrad.exe} and
\texttt{configs/full.cfg} are authoritative for shipping presets.
}
